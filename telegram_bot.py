...
"""
    ...
    def automation_worker(self):
        ...
        # Schedule uploads
        schedule.every().day.at("00:00").do(self.youtube_bot.reset_daily)
        
        # Tech uploads
        tech_times = ["08:00", "12:00", "16:00", "20:00", "23:00"]
        for time_slot in tech_times:
            schedule.every().day.at(time_slot).do(self.scheduled_tech_upload)
        
        # Entertainment uploads
        entertainment_times = ["10:00", "14:00", "18:00", "21:00", "23:30"]
        for time_slot in entertainment_times:
            schedule.every().day.at(time_slot).do(self.scheduled_entertainment_upload)
    ...
    def scheduled_tech_upload(self):
        """Scheduled tech upload with notification"""
        if not self.is_running:
            return
            
        self.send_message("🔧 <b>Scheduled Tech Upload Starting...</b>")
        success = self.youtube_bot.process_tech_video()
        
        if not success:
            self.send_message("❌ <b>Scheduled tech upload failed!</b>")
    
    def scheduled_entertainment_upload(self):
        """Scheduled entertainment upload with notification"""
        if not self.is_running:
            return
            
        self.send_message("🎬 <b>Scheduled Entertainment Upload Starting...</b>")
        success = self.youtube_bot.process_entertainment_video()
        
        if not success:
            self.send_message("❌ <b>Scheduled entertainment upload failed!</b>")
    
    def run_forever(self):
        """Run Telegram bot forever"""
        print("🚀 Starting Telegram YouTube Bot...")
        
        # Start automation worker in background
        automation_thread = threading.Thread(target=self.automation_worker, daemon=True)
